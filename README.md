...
## Core capabilities
* **Layer 2 / Layer 3 Discovery:** Utilizes a passive MACRAW engine to infer subnets without transmitting packets, followed by GARP (Gratuitous ARP) collision checks for stealthy IP auto-assignment.
...
* **Target Stress Testing (Fuzzer):** Injects anomalous and malformed Modbus TCP payloads to evaluate the robustness of the target PLC's internal TCP stack and application-layer parsers.
...
2.  Ensure the `M5StickCPlus2` and `Ethernet` libraries are installed via the Library Manager.
